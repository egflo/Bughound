...
<html>
	<head>
		<meta charset="UTF-8">
		<title>List of Status Types</title>
	...
	<h1>List of Status Types</h1>
			...
			while($row=mysqli_fetch_row($result)) {
				...
				<td>
					<form action='edit_status.php' method='post'>
						<input id='stat_id' name='stat_id' type='hidden' value = %s />
		...
		<script language=Javascript>
			...
			function search() {
				window.location.replace("search_status.php");
			}
			function create(){
				window.location.replace("create_status.php");
			}
			function cancel(){
