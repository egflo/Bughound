...
<html>
	<head>
		<meta charset="UTF-8">
		<title>List of priority Types</title>
	...
	<h1>List of priority Types</h1>
		<?php
			...
			while($row=mysqli_fetch_row($result)) {
				...
				<td>%d</td><td>%s</td>
				<td>
					<form action='edit_priority.php' method='post'>
						<input id='pr_id' name='pr_id' type='hidden' value = %s />
						<input type='submit' name = 'submit' value='Edit' />
					</form>
				</td>
				<td>
					<form action='delete_priority.php' method='post' >
		...
		<script language=Javascript>
			...
			for(var i = 0; i<table.rows.length;i++)
			{
				table.rows[i].onclick = function()
				{
					...
					document.getElementById("priority").submit();
				}
			}
			function search() {
				window.location.replace("search_priority.php");
			}
			function create(){
				window.location.replace("create_priority.php");
			}
			function cancel(){
